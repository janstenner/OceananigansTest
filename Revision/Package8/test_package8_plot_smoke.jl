...
@testset "Package-8 pooled Pareto plot" begin
    ...
    for replicate in P8_REPLICATES, (threshold_index, threshold) in enumerate(fixture_thresholds)
    ...
    @test sparse[:validation_matching] <= P8_QUALITY_THRESHOLD
    @test sparse[:active_inputs] == minimum(
        record[:active_inputs] for record in front
        if record[:validation_matching] <= P8_QUALITY_THRESHOLD
    )
    @test isnothing(select_sparse_test_candidate([
        Dict{Symbol, Any}(
            :validation_matching => 2 * P8_QUALITY_THRESHOLD,
            :active_inputs => 1,
            :active_groups => 1,
            :update => 0,
            :run_id => "nr",
    ...
    mktempdir() do directory
        ...
        episode = (
            ...
            mirror = false,
            offset = 0,
            evaluation_seed = P8_MASTER_SEED + 1,
            episode = 1,
            rewards = collect(range(-3.0, -2.0; length = P8_TEST_STEPS)),
            state_nusselt = collect(range(3.0, 2.0; length = P8_TEST_STEPS)),
            simulation_times = collect(1.0:P8_TEST_STEPS),
            actions = zeros(Float32, P8_TEST_STEPS, 12),
        )
        test_csv = write_test_csv(joinpath(directory, "test", "test_episodes.csv"), [episode])
        @test length(readlines(test_csv)) == P8_TEST_STEPS + 1
...
println("package8-plot-smoke-ok")
