...
results_directory="${P8_RESULTS_DIR:-${script_directory}/results}"
...
Without filters, starts all eight Package-8 configurations at their configured
...
Options:
  ...
  --results-dir PATH    Override the Package-8 result root.
...
((${#variant_rows[@]} > 0)) || { echo "No Package-8 variants selected." >&2; exit 2; }
...
for configuration in "${selected_configurations[@]}"; do
    ...
    for strength in "${strengths[@]}"; do
        ...
        if [[ "${analysis_only}" == false ]]; then
            for replicate in 1 2 3; do
                ...
                session="p8_${experiment_id}_${configuration_tag}_${strength_session_tag}_${replicate_tag}"
                command=(
    ...
    session="p8_${experiment_id}_${configuration_tag}_analyze"
    analysis_command=(
