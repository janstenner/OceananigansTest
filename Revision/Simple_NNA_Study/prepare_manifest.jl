...
function prepare_manifest(options)
    ...
    length(unique(job.id for job in jobs)) == length(jobs) || error("Duplicate Simple-NNA run IDs.")
    length(unique(job.relative_path for job in jobs)) == length(jobs) || error("Duplicate Simple-NNA run paths.")
    atomic_save(
        options.output;
        ...
        seed_plan = [seed_plan(replicate) for replicate in SNN_REPLICATES],
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    ...
    if !isnothing(options)
        if options.print_variants
            ...
            println("Wrote $(length(jobs)) training jobs for $(length(variants)) Simple-NNA variant(s).")
        end
    end
end
