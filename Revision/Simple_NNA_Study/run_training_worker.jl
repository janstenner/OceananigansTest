...
const SNN_DIRECTORY = @__DIR__
const SNN_PROJECT_ROOT = normpath(joinpath(SNN_DIRECTORY, "..", ".."))
const SNN_DISTILLATION_DIRECTORY = joinpath(SNN_PROJECT_ROOT, "Revision", "Expert_Apprentice_Distillation")
const SNN_DEFAULT_RESULTS_ROOT = joinpath(SNN_DIRECTORY, "results")

function worker_usage(io::IO = stdout)
    println(io, """
    Simple-NNA Varying-IC training worker.
...
function parse_worker_arguments(arguments)
    values = Dict{String, Any}(
        ...
        "strength" => nothing,
        "replicate" => nothing,
        "results_dir" => SNN_DEFAULT_RESULTS_ROOT,
        "retry_failed" => false,
    ...
    updates = isnothing(smoke_updates) ? SNN_UPDATES : smoke_updates
    return (
...
function configure_worker_environment!(options)
    ...
    expert_path = joinpath(SNN_DISTILLATION_DIRECTORY, "experts", "varying", "agent.jld2")
    ...
    ENV["DISTILLATION_WORKER_DIRECTORY"] = joinpath(SNN_DISTILLATION_DIRECTORY, "worker_results", "varying")
...
function archive_config(options, training_config, expert_path, inputs, initial_hash)
    ...
    return Dict{Symbol, Any}(
        ...
        :scientific_scope => isnothing(options.smoke_updates) ? :simple_nna : :smoke_test,
        :protocol => :varying,
        ...
        :master_seed => SNN_MASTER_SEED,
        :apprentice_seed => job.apprentice_seed,
        :batch_order_seed => job.batch_seed,
        :regression_learning_rate => SNN_LEARNING_RATE,
        :apprentice_architecture => SNN_APPRENTICE_ARCHITECTURE,
        :nna_scale => SNN_NNA_SCALE,
        :hidden_size => SNN_HIDDEN_SIZE,
        :mat_actor_parameter_count => SNN_MAT_ACTOR_PARAMETER_COUNT,
        :apprentice_parameter_count => SNN_PARAMETER_COUNT,
        :parameter_count_difference => SNN_PARAMETER_COUNT - SNN_MAT_ACTOR_PARAMETER_COUNT,
        :parameter_count_relative_difference =>
            (SNN_PARAMETER_COUNT - SNN_MAT_ACTOR_PARAMETER_COUNT) / SNN_MAT_ACTOR_PARAMETER_COUNT,
        ...
        :evaluation_interval => SNN_EVALUATION_INTERVAL,
...
function run_loaded_worker(options, directory, expert_path)
    job = options.job
    Float64(learning_rate) == SNN_LEARNING_RATE || error(
        "Loaded learning rate $learning_rate does not match Simple-NNA value $SNN_LEARNING_RATE.",
    )
    EXPERT_APPRENTICE_ARCHITECTURE === SNN_APPRENTICE_ARCHITECTURE || error(
        "Loaded apprentice architecture $EXPERT_APPRENTICE_ARCHITECTURE does not match $(SNN_APPRENTICE_ARCHITECTURE).",
    )
    SIMPLE_NNA_SCALE == SNN_NNA_SCALE || error("Simple-NNA scale mismatch: $SIMPLE_NNA_SCALE.")
    SIMPLE_NNA_HIDDEN_SIZE == SNN_HIDDEN_SIZE || error("Simple-NNA hidden-size mismatch: $SIMPLE_NNA_HIDDEN_SIZE.")
    MAT_APPRENTICE_ACTOR_PARAMETER_COUNT == SNN_MAT_ACTOR_PARAMETER_COUNT || error(
        "MAT actor parameter-count mismatch: $MAT_APPRENTICE_ACTOR_PARAMETER_COUNT.",
    )
    SIMPLE_NNA_PARAMETER_COUNT == SNN_PARAMETER_COUNT || error(
        "Simple-NNA parameter-count mismatch: $SIMPLE_NNA_PARAMETER_COUNT.",
    )
    ...
    training_config = ApprenticeTrainingConfig(
        ...
        regularization_strength = job.regularization_strength,
        validation_batch_size = SNN_VALIDATION_BATCH_SIZE,
    ...
    schedule = CandidateSchedule(
        start_update = 0,
        evaluation_interval = SNN_EVALUATION_INTERVAL,
        garbage_collection_interval = SNN_GARBAGE_COLLECTION_INTERVAL,
        resume_interval = SNN_RESUME_INTERVAL,
    )
    ...
    println("Simple NNA worker $(job.id)")
    ...
    println("  actor: Dense 360->$SNN_HIDDEN_SIZE->$SNN_HIDDEN_SIZE->1, nna_scale=$SNN_NNA_SCALE, parameters=$SNN_PARAMETER_COUNT (MAT actor target $SNN_MAT_ACTOR_PARAMETER_COUNT)")
    println("  updates/batches: $(job.updates) / $SNN_BATCH_SIZE, validation $SNN_VALIDATION_BATCH_SIZE")
    ...
    try
        result = train_apprentice!(
            ...
            threshold_importance_mode = :max_input_l1,
            threshold_minimum_active_groups = 1,
            threshold_pareto_scope = :simple_nna_thresholds,
            require_threshold_group_reduction = true,
            resume,
        )
...
if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    ...
    if !isnothing(options)
        ...
        include(joinpath(SNN_DISTILLATION_DIRECTORY, "Expert_Apprentice.jl"))
