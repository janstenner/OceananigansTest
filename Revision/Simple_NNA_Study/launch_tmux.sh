...
results_directory="${SNN_RESULTS_DIR:-${script_directory}/results}"
...
Each call starts exactly one Simple-NNA configuration at its configured
three-strength grid: nine training workers and one analysis/wait worker.

Options:
  --config NAME         Required; one of go-gc, go-sc, gr-gc, gr-sc.
  ...
  --results-dir PATH    Override the Simple-NNA result root.
...
[[ -n "${configuration_selection}" ]] || {
    echo "--config is required; launch go-gc, go-sc, gr-gc, and gr-sc separately." >&2
    exit 2
}
[[ "${configuration_selection,,}" != all ]] || {
    echo "--config all is intentionally disabled; launch the four configurations separately." >&2
    exit 2
}
...
((${#variant_rows[@]} > 0)) || { echo "No Simple-NNA variants selected." >&2; exit 2; }
...
for configuration in "${selected_configurations[@]}"; do
    ...
    for strength in "${strengths[@]}"; do
        ...
        if [[ "${analysis_only}" == false ]]; then
            for replicate in 1 2 3; do
                ...
                session="snn_${experiment_id}_${configuration_tag}_${strength_session_tag}_${replicate_tag}"
                command=(
    ...
    session="snn_${experiment_id}_${configuration_tag}_analyze"
    analysis_command=(
