...
scriptname = "Revision_VaryingIC_IPPO"

#dir variable
dirpath = get(
    ENV,
    "REVISION_RUN_DIRECTORY",
    joinpath(@__DIR__, "outputs", scriptname),
)
mkpath(dirpath)
...
seed = if haskey(ENV, "REVISION_RUN_SEED")
    parse(Int, ENV["REVISION_RUN_SEED"])
else
    rand(0:99_999)
end
...
rng = StableRNG(seed)
initial_condition_rng = StableRNG(seed + 1)
initial_condition_split = :train
Random.seed!(seed)
y = 0.99f0
p = 0.95f0
...
function initialize_setup(;use_random_init = false)
    ...
    # One shared actor/critic pair is applied to every actuator column:
    # parameter-sharing IPPO rather than one independently parameterized policy per actuator.
    ...
    global hook = GeneralHook(min_best_episode = min_best_episode,
                collect_NNA = false,
                generate_random_init = () -> first(generate_random_init()),
...
function generate_random_init(;
    ...
    return (
        ...
        initial_condition.base_seed,
        initial_condition.mirror,
        initial_condition.offset,
    )
end
...
function train(use_random_init = true; visuals = false, num_steps = 1600, inner_loops = 5, outer_loops = 50)
    ...
    if use_random_init
        hook.generate_random_init = () -> first(generate_random_init())
    else
