...
results_directory="${MAT_EXPERT_RA5E4_RESULTS_DIR:-${script_directory}/results_ra5e4}"
run_count=10
master_seed="${MAT_EXPERT_RA5E4_MASTER_SEED:-20260901}"
episodes=""
threshold=""
preview=false
openblas_threads=3
...
usage() {
    cat <<'EOF'
Usage: Revision/MAT_expert_training/launch_tmux_ra5e4.sh [options]

Starts independent Varying-IC MAT training runs at Ra=5e4. Choose exactly one
stop rule:
...
Options:
  ...
  --results-dir PATH        Result root (default: results_ra5e4).
...
Environment:
  ...
  MAT_EXPERT_RA5E4_RESULTS_DIR    Result root
  MAT_EXPERT_RA5E4_MASTER_SEED    Master seed (default: 20260901)
EOF
}
...
worker_script="${script_directory}/run_ra5e4_training_worker.jl"
common_arguments=(
    --runs "${run_count}"
    --master-seed "${master_seed}"
...
for ((index = 1; index <= run_count; index++)); do
    ...
    session="pmat_ra5e4_${padded_index}"
    start_session \
        "${session}" "${index}" \
...
if [[ "${preview}" == true ]]; then
    echo "Preview complete: ${run_count} Ra=5e4 training sessions would be started."
else
    echo
    echo "Submitted ${run_count} Ra=5e4 training sessions."
    echo "Inspect sessions: tmux ls"
    echo "Attach:           tmux attach -t pmat_ra5e4_01"
    echo "Follow a log:     tail -f ${launch_directory}/pmat_ra5e4_01.log"
