...
function usage()
    println(
        """
        Usage: julia --project=. Revision/MAT_expert_training/run_ra5e4_training_worker.jl [options]
...
function parse_options(arguments)
    options = Dict{String, Any}(
        "run_index" => nothing,
        "runs" => 10,
        "master_seed" => MATExpertTraining.DEFAULT_RA5E4_MASTER_SEED,
        "results_dir" => MATExpertTraining.DEFAULT_RA5E4_RESULTS_DIRECTORY,
        "episodes" => nothing,
        "threshold" => nothing,
        "prepare_only" => false,
        "preview" => false,
...
function main(arguments = ARGS)
    ...
    if options["prepare_only"] || options["preview"]
        experiment = MATExpertTraining.freeze_ra5e4_experiment!(
        ...
        println(options["preview"] ? "Ra=5e4 experiment preview:" : "Frozen Ra=5e4 experiment:")
        println(
    ...
    return MATExpertTraining.run_ra5e4_training_worker(
