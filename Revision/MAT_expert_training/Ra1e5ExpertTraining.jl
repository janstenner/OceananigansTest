# Included inside MATExpertTraining. The Ra=1e5 path deliberately reuses the
...
ra1e5_protocol_directory(results_directory) = joinpath(results_directory, "varying")
ra1e5_manifest_path(results_directory) = joinpath(results_directory, "experiment_manifest.jld2")
ra1e5_stop_signal_path(results_directory) = joinpath(
    ra1e5_protocol_directory(results_directory),
    "stop_signal.jld2",
)
ra1e5_best_checkpoint_path(results_directory) = joinpath(
    ra1e5_protocol_directory(results_directory),
    "best_so_far.jld2",
)
ra1e5_expert_path(results_directory) = joinpath(
    ra1e5_protocol_directory(results_directory),
    "expert.jld2",
)
ra1e5_worker_directory(results_directory, candidate) = joinpath(
    results_directory,
    "runs",
    candidate.run_id,
)
ra1e5_resume_path(results_directory, candidate) = joinpath(
    ra1e5_worker_directory(results_directory, candidate),
    "resume",
    "latest.jld2",
)
ra1e5_final_path(results_directory, candidate) = joinpath(
    ra1e5_worker_directory(results_directory, candidate),
    "final.jld2",
)
ra1e5_failure_path(results_directory, candidate) = joinpath(
    ra1e5_worker_directory(results_directory, candidate),
    "failure.jld2",
)

function normalize_ra1e5_stop_mode(value)
    mode = Symbol(lowercase(replace(string(value), '-' => '_')))
    mode in (:episodes, :threshold) || throw(ArgumentError(
        "Ra=1e5 stop mode must be 'episodes' or 'threshold', got '$value'.",
    ))
    return mode
end

function ra1e5_candidates(master_seed::Integer, run_count::Integer)
    ...
    return [
        begin
            ...
            (
                run_index = index,
                run_id = "ra1e5_r$(lpad(string(index), width, '0'))_" *
...
function ra1e5_dict_value(mapping, key::Symbol)
    haskey(mapping, key) && return mapping[key]
    string_key = string(key)
    haskey(mapping, string_key) && return mapping[string_key]
    error("Missing '$key' in Ra=1e5 corpus metadata.")
end

function validate_ra1e5_corpus(path::AbstractString = RA1E5_CORPUS_PATH)
    isfile(path) || error(
        "Missing Ra=1e5 Varying-IC corpus: $path. Finish corpus generation before launching training.",
    )
    JLD2.jldopen(path, "r") do file
        haskey(file, "corpus") || error("Ra=1e5 corpus has no 'corpus' entry: $path")
        haskey(file, "simulation_config") || error(
            "Ra=1e5 corpus has no 'simulation_config' entry: $path",
        )
        config = read(file, "simulation_config")
        observed_ra = Float64(ra1e5_dict_value(config, :Ra))
        observed_ra == RA1E5_RAYLEIGH || error(
            "Ra=1e5 corpus reports Ra=$observed_ra instead of $RA1E5_RAYLEIGH.",
        )
        corpus = read(file, "corpus")
        for split in (:train, :validation, :test)
            records = ra1e5_dict_value(corpus, split)
            isempty(records) && error("Ra=1e5 corpus split '$split' is empty: $path")
        end
    end
    return source_hash(path)
end

function ra1e5_config_fingerprint(;
    master_seed,
    run_count,
    stop_mode,
    episode_limit,
    ...
    fields = (
        "schema=$(RA1E5_SCHEMA_VERSION)",
        "protocol=$(RA1E5_PROTOCOL)",
        "ra=$(RA1E5_RAYLEIGH)",
        "window=$(RA1E5_WINDOW)",
...
function build_ra1e5_experiment(;
    master_seed::Integer = DEFAULT_RA1E5_MASTER_SEED,
    run_count::Integer,
    stop_mode,
    ...
    mode = normalize_ra1e5_stop_mode(stop_mode)
    run_count > 0 || throw(ArgumentError("run_count must be positive."))
    if mode === :episodes
        episode_limit >= RA1E5_WINDOW || throw(ArgumentError(
            "The fixed episode limit must be at least $RA1E5_WINDOW so a rolling-100 best candidate exists.",
        ))
    ...
    isfile(RA1E5_RUN_FILE_PATH) || error("Missing Ra=1e5 run file: $RA1E5_RUN_FILE_PATH")
    isfile(RA1E5_CORPUS_SOURCE_PATH) || error(
        "Missing Ra=1e5 corpus source: $RA1E5_CORPUS_SOURCE_PATH",
    )
    corpus_sha = if preview && !isfile(RA1E5_CORPUS_PATH)
        "pending-corpus-generation"
    else
        validate_ra1e5_corpus()
    end
    corpus_source_sha = source_hash(RA1E5_CORPUS_SOURCE_PATH)
    run_file_sha = source_hash(RA1E5_RUN_FILE_PATH)
    candidates = ra1e5_candidates(master_seed, run_count)
    fingerprint = ra1e5_config_fingerprint(
        ;
        master_seed,
        run_count,
        stop_mode = mode,
    ...
    return (
        protocol = RA1E5_PROTOCOL,
        rayleigh = RA1E5_RAYLEIGH,
        rolling_window = RA1E5_WINDOW,
        master_seed = Int(master_seed),
        run_count = Int(run_count),
        stop_mode = mode,
        episode_limit = normalized_episode_limit,
        threshold = normalized_threshold,
        corpus_path = abspath(RA1E5_CORPUS_PATH),
        corpus_sha256 = corpus_sha,
        corpus_source_path = abspath(RA1E5_CORPUS_SOURCE_PATH),
        corpus_source_sha256 = corpus_source_sha,
        run_file_path = abspath(RA1E5_RUN_FILE_PATH),
        run_file_sha256 = run_file_sha,
...
function freeze_ra1e5_experiment!(;
    results_directory::AbstractString = DEFAULT_RA1E5_RESULTS_DIRECTORY,
    master_seed::Integer = DEFAULT_RA1E5_MASTER_SEED,
    run_count::Integer,
    stop_mode,
    episode_limit::Integer = 0,
    ...
    experiment = build_ra1e5_experiment(
        ;
        master_seed,
        run_count,
        stop_mode,
    ...
    path = ra1e5_manifest_path(results_directory)
    return with_lock(path * ".lock"; wait_seconds = 60.0) do
        if isfile(path)
            existing = JLD2.load(path, "experiment")
            existing == experiment || error(
                "Existing Ra=1e5 experiment manifest differs from this launch: $path. " *
        ...
        atomic_jldsave(
            path;
            schema_version = RA1E5_SCHEMA_VERSION,
            status = "frozen",
...
function ra1e5_candidate(experiment, run_index::Integer)
    matches = [candidate for candidate in experiment.candidates if
               candidate.run_index == Int(run_index)]
    length(matches) == 1 || error("No unique Ra=1e5 candidate for run index $run_index.")
    return only(matches)
end

function include_ra1e5_run_file!(candidate, runtime_directory)
    ENV["REVISION_RUN_SEED"] = string(candidate.run_seed)
    ENV["REVISION_RUN_DIRECTORY"] = runtime_directory
    mkpath(runtime_directory)
    Base.include(@__MODULE__, RA1E5_RUN_FILE_PATH)
    hook.is_display_on_exit = false
    hook.display_after_episode = false
    return nothing
end

function ra1e5_training_seeds()
    # VaryingICCorpus_Ra1e5.jl has already loaded the corpus for sampling. Use
    # that object instead of briefly loading a second full copy per worker.
    split = ra1e5_dict_value(CORPUS, :train)
    seeds = sort!(Int.(collect(keys(split))))
    isempty(seeds) && error("Ra=1e5 training corpus is empty.")
    return seeds
end

function configure_ra1e5_episode_initializer!(candidate, observed_trace)
    seeds = ra1e5_training_seeds()
    ...
    for (index, observed) in enumerate(observed_trace)
        ...
        observed == expected || error(
            "Stored Ra=1e5 IC trace differs at episode $index: expected $expected, got $observed.",
        )
    end
    hook.generate_random_init = () -> begin
        ...
        observed == choice || error("Ra=1e5 run file did not reproduce planned IC $choice.")
...
function load_ra1e5_training_state(candidate, results_directory, fingerprint)
    path = ra1e5_resume_path(results_directory, candidate)
    if isfile(path)
...
function save_ra1e5_resume!(candidate, results_directory, experiment,
                            episodes_completed, initial_condition_trace,
                            elapsed_seconds)
    atomic_jldsave(
        ra1e5_resume_path(results_directory, candidate);
        schema_version = RA1E5_SCHEMA_VERSION,
        status = "running",
        protocol = RA1E5_PROTOCOL,
        rayleigh = RA1E5_RAYLEIGH,
...
ra1e5_criterion_value(rewards) = length(rewards) < RA1E5_WINDOW ? NaN :
    mean(@view rewards[(end - RA1E5_WINDOW + 1):end])

function save_ra1e5_best_so_far!(candidate, results_directory, experiment,
                                 episodes_completed, initial_condition_trace,
                                 elapsed_seconds)
    metric = ra1e5_criterion_value(hook.rewards)
    isfinite(metric) || return false
    protocol_directory = ra1e5_protocol_directory(results_directory)
    mkpath(protocol_directory)
    checkpoint_path = ra1e5_best_checkpoint_path(results_directory)
    expert_path = ra1e5_expert_path(results_directory)
    return with_lock(
        ...
        if isfile(checkpoint_path)
            existing = JLD2.jldopen(checkpoint_path, "r") do file
                string(read(file, "status")) == "best_so_far" || error(
                    "Invalid Ra=1e5 best-so-far status: $checkpoint_path",
                )
                string(read(file, "config_fingerprint")) == experiment.config_fingerprint ||
                    error("Ra=1e5 best-so-far configuration mismatch.")
...
function read_ra1e5_stop_signal(results_directory, fingerprint)
    path = ra1e5_stop_signal_path(results_directory)
    isfile(path) || return nothing
    signal = JLD2.load(path)
    string(signal["config_fingerprint"]) == fingerprint || error(
        "Ra=1e5 stop-signal configuration mismatch: $path",
    )
    return signal
end

function publish_ra1e5_threshold!(candidate, results_directory, experiment,
                                  metric, episodes_completed)
    directory = ra1e5_protocol_directory(results_directory)
    mkpath(directory)
    return with_lock(joinpath(directory, ".candidate.lock"); wait_seconds = 60.0) do
        existing = read_ra1e5_stop_signal(results_directory, experiment.config_fingerprint)
        !isnothing(existing) && return false
        atomic_jldsave(
            ra1e5_stop_signal_path(results_directory);
            schema_version = RA1E5_SCHEMA_VERSION,
...
function ra1e5_final_complete(path, candidate, fingerprint)
...
function save_ra1e5_final!(candidate, results_directory, experiment,
                           episodes_completed, initial_condition_trace,
    ...
    atomic_jldsave(
        ra1e5_final_path(results_directory, candidate);
        schema_version = RA1E5_SCHEMA_VERSION,
        status = "complete",
        completion_reason,
        ...
        rolling_window = RA1E5_WINDOW,
        episodes_completed = Int(episodes_completed),
        final_criterion_value = ra1e5_criterion_value(hook.rewards),
...
function save_ra1e5_failure!(candidate, results_directory, experiment, message)
    atomic_jldsave(
        ra1e5_failure_path(results_directory, candidate);
        schema_version = RA1E5_SCHEMA_VERSION,
        status = "failed",
        protocol = RA1E5_PROTOCOL,
        rayleigh = RA1E5_RAYLEIGH,
...
function run_ra1e5_training_worker(;
    run_index::Integer,
    results_directory::AbstractString = DEFAULT_RA1E5_RESULTS_DIRECTORY,
    master_seed::Integer = DEFAULT_RA1E5_MASTER_SEED,
    run_count::Integer,
    ...
    experiment = freeze_ra1e5_experiment!(
        ;
        results_directory,
        master_seed,
        run_count,
    ...
    candidate = ra1e5_candidate(experiment, run_index)
    output_directory = ra1e5_worker_directory(results_directory, candidate)
    mkpath(output_directory)
    lock = acquire_lock(joinpath(output_directory, ".worker.lock"))
    try
        final_path = ra1e5_final_path(results_directory, candidate)
        if ra1e5_final_complete(final_path, candidate, experiment.config_fingerprint)
            println("Complete Ra=1e5 result already exists; skipping $(candidate.run_id).")
            return final_path
        end

        include_ra1e5_run_file!(candidate, joinpath(output_directory, "runtime"))
        state = load_ra1e5_training_state(
            candidate,
            results_directory,
        ...
        configure_ra1e5_episode_initializer!(candidate, initial_condition_trace)
        episodes_completed = state.episodes_completed
        episodes_completed == length(hook.rewards) || error(
            "Ra=1e5 episode count and stored reward count differ.",
        )
        length(initial_condition_trace) == episodes_completed || error(
            "Ra=1e5 IC trace and episode count differ.",
        )
        ...
        initial_best_updated = save_ra1e5_best_so_far!(
            candidate,
            results_directory,
            experiment,
            episodes_completed,
        ...
        stop_signal = experiment.stop_mode === :threshold ?
            read_ra1e5_stop_signal(results_directory, experiment.config_fingerprint) : nothing
        if experiment.stop_mode === :threshold && isnothing(stop_signal) &&
           isfinite(metric) && metric > experiment.threshold
            won = publish_ra1e5_threshold!(
                candidate,
                results_directory,
                experiment,
                metric,
            ...
            won && println("Published Ra=1e5 threshold stop from resumed state.")
            stop_signal = read_ra1e5_stop_signal(
                results_directory,
                experiment.config_fingerprint,
            )
        end
        println(
            "Starting Ra=1e5 $(candidate.run_id) from episode $episodes_completed; " *
            "stop_mode=$(experiment.stop_mode), " *
            (experiment.stop_mode === :episodes ?
                "episode_limit=$(experiment.episode_limit)" :
                "threshold=$(experiment.threshold) after rolling window $(RA1E5_WINDOW)") *
        ...
        while true
            ...
            length(initial_condition_trace) == episodes_completed || error(
                "Ra=1e5 IC trace and episode count differ after training.",
            )
            save_ra1e5_resume!(
                candidate,
                results_directory,
            ...
            metric = ra1e5_criterion_value(hook.rewards)
            best_updated = save_ra1e5_best_so_far!(
                candidate,
                results_directory,
                experiment,
                episodes_completed,
            ...
            reached = experiment.stop_mode === :threshold && isfinite(metric) &&
                metric > experiment.threshold
            @printf(
                "[%s] protocol=varying_ra1e5 run=%d/%d id=%s episode=%d reward=%.6f rolling_100=%.6f stop_mode=%s target=%s reached=%s global_best_updated=%s\n",
                now(),
                candidate.run_index,
            ...
            if reached
                ...
                won && println("Published Ra=1e5 threshold winner $(candidate.run_id).")
            end
            stop_signal = experiment.stop_mode === :threshold ?
                read_ra1e5_stop_signal(results_directory, experiment.config_fingerprint) : nothing
        end
        ...
        failure_path = ra1e5_failure_path(results_directory, candidate)
        isfile(failure_path) && rm(failure_path; force = true)
        println(
            "Saved Ra=1e5 final checkpoint for $(candidate.run_id): " *
            "episodes=$episodes_completed, rolling_100=$(ra1e5_criterion_value(hook.rewards)).",
        )
        flush(stdout)
        return final_path
    catch error_value
        message = sprint(showerror, error_value, catch_backtrace())
        save_ra1e5_failure!(candidate, results_directory, experiment, message)
        rethrow(error_value)
    finally
