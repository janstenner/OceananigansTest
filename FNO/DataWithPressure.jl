...
colorscale_pressure = [[0, "rgb(34, 74, 168)"], [0.5, "rgb(224, 224, 180)"], [1.0, "rgb(156, 33, 11)"], ]

layout = Layout(
        plot_bgcolor="#f1f3f7",
        coloraxis = attr(cmin = 1, cmid = 2.5, cmax = 3, colorscale = colorscale),
        coloraxis_pressure = attr(cmin = -3, cmid = 0, cmax = 3, colorscale = colorscale_pressure),
)
...
function generate_data(one_by_one = false)
    ...
    global sim_results = zeros(Float32,Nx,Nz,5,frames_to_collect)

    for i in 1:start_point+frames_to_collect
        global simulation.stop_time = Δt_snap*i
        ...
        if i>start_point
            j = i - start_point
            sim_results[:,:,1,j] = model.pressures.pHY′[1:Nx,1,1:Nz]
            sim_results[:,:,2,j] = model.pressures.pNHS[1:Nx,1,1:Nz]

            sim_results[:,:,3,j] = model.tracers.b[1:Nx,1,1:Nz]
            sim_results[:,:,4,j] = model.velocities.u[1:Nx,1,1:Nz]
            sim_results[:,:,5,j] = model.velocities.w[1:Nx,1,1:Nz]
        end
    ...
    for i in 1:size(sim_results)[4]
        p = make_subplots(rows=2, cols=2)
        add_trace!(p, heatmap(z=sim_results[:,:,1,i]', coloraxis="coloraxis_pressure"), col = 1, row=1)
        add_trace!(p, heatmap(z=sim_results[:,:,2,i]', coloraxis="coloraxis_pressure"), col = 2, row=1)
        add_trace!(p, heatmap(z=sim_results[:,:,1,i]'+sim_results[:,:,2,i]', coloraxis="coloraxis_pressure"), col = 1, row=2)
        add_trace!(p, heatmap(z=sim_results[:,:,3,i]', coloraxis="coloraxis"), col = 2, row=2)
        relayout!(p, layout.fields)
        savefig(p, dirpath * "/compare_frames//a$(lpad(string(i), 5, '0')).png"; width=1200, height=1200)
    end
